...
for _ in range(Q):
    command = input().split()
    cmd, t, x, n = -1, -1, -1, -1  #초기화를 해야지 입력에 없어도 괜찮은 거다! 
    name = ""
    cmd = int(command[0])
    if cmd == 100:
        t, x, name = command[1:]
    ...
    queries.append(Query(cmd, t, x, name, n)) #쿼리(클래스)를 배열에 넣기
...
for name in names:
    ...
    for q in p_queries[name]:
        time_to_removed = 0
        if q.t < entry_time[name]: # 만약 초밥이 사람이 등장하기 전에 미리 주어진 상황이라면
            ...
            time_to_removed = entry_time[name] + additionl_time #입장시간+ 몇초뒤 만나는지
        ...
        # 초밥이 사라지는 시간 중 가장 늦은 시간을 업데이트 합니다. (갱신)
...
queries.sort(key=lambda q: (q.t, q.cmd)) # 정렬순서 t-> cmd
